...
## Release_3.7.1_20251113_build_A
...
- `webapp-pc` 子模块页面 bug 修复。
  - 配置仓库 -> 配置节点管理初始化节点对话框表单标签显示错误。
  - 配置仓库 -> 配置节点编辑器国际化节点编辑子面板语言验证消息显示错误。
...
## Release_3.7.0_20250817_build_A

### 功能构建

- `webapp-mobile` 子模块功能优化。
  - 增加统一的登录背景图片，并更新可视化器，使其渲染登录页面时，默认使用该图片。
  - 增加统一的错误背景图片，并更新可视化器，使其渲染错误页面时，默认使用该图片。
...
- `component-api` 子模块功能调整。
  - 调整 `src/api/settingrepo` 目录中的部分类型，以设备新版本网络中台。

- `webapp-pc` 子模块页面优化。
  - 配置仓库 -> 配置节点编辑器文本节点子编辑器增加刷新功能。
  - 学习笔记 -> 笔记管理主面板资料页签文件上传对话框已选择文件清除逻辑优化。
  - 资产管理 -> 资产BOM管理主面板资料页签文件上传对话框已选择文件清除逻辑优化。
...
## Release_3.6.0_20250809_build_C
...
## Release_3.6.0_20250809_build_B
...
- `webapp-pc` 子模块页面 bug 修复。
  - 资金管理 -> 资金报告主面板银行卡页签主面板锚点银行卡处理逻辑异常。
  - 资金管理 -> 资金报告主面板资金变更页签主面板锚点资金变更处理逻辑异常。
  - 资金管理 -> 资金变更记录右侧票据面板主表格内容列，图片预览时显示异常。

- `webapp-pc` 子模块组件 bug 修复。
  - `GeneralChartPanel` 组件在长度或宽度为 `0` 的容器中，初始化时控制台报错。
