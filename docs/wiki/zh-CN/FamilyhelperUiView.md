...
- 初始化、构建与脚本格式命令：模块内部的 `README.md`。
- VIM 5 构型中的 `scripts/build.js` 原理：[VIM 5 构建脚本与 build.js](./FamilyhelperUiViewBuildScript.md)。
- 根工程打包与部署：Wiki 中 [从源码编译](./CompileBySource.md)、[配置目录](./ConfDirectory.md) 等条目。
- 除上述专题外，本 Wiki 仍可继续补充：
  vim 模型说明、各 `webapp-*` 的结构与规范、与后端的接口与环境配置、可视化表单等业务域文档；篇名与目录将随撰写进度更新。
