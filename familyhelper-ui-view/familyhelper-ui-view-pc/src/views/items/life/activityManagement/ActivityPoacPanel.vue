<template>
  <div class="activity-poac-panel-container">
...
import { childForActivityDisp } from '@/api/life/poac';
import { removePermission, upsertPermission } from '@/api/life/activity';
import resolveResponse from '@/util/response';

// noinspection JSAnnotator
export default {
  name: 'ActivityPoacPanel',
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.lookupChildForActivity();
    },
    lookupChildForActivity() {
      this.loading = true;
      resolveResponse(childForActivityDisp(
        this.activityId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.current_page > res.total_pages && res.total_pages > 0) {
            return resolveResponse(childForActivityDisp(
    ...
    handleDeleteButtonClicked(index, row) {
      ...
      Promise.resolve(row)
        .then((res) => this.$confirm(
          `此操作将删除 ${row.account.display_name}(${row.account.key.string_id}) 对该活动的权限。<br>是否继续?`,
          '提示', {
        ...
        .then((res) => resolveResponse(
          removePermission(this.activityId, res.key.user_string_id),
        ))
        .then(() => {
          this.$message({
            showClose: true,
