<template>
  <div class="activity-template-poatac-panel-container">
...
import { childForActivityTemplateDisp } from '@/api/life/poatac';
import { upsertActivityPermission, removeActivityPermission } from '@/api/life/activityTemplate';
import resolveResponse from '@/util/response';

// noinspection JSAnnotator
export default {
  name: 'ActivityTemplatePoatacPanel',
  ...
  methods: {
    ...
    handleUpsertButtonClicked() {
      resolveResponse(upsertActivityPermission(
    ...
    handleDeleteButtonClicked(index, row) {
      ...
      Promise.resolve(row)
        .then((res) => this.$confirm(
          `此操作将删除 ${row.account.display_name}(${row.account.key.string_id}) 对该活动模板生成的活动的权限。<br>是否继续?`,
          '提示', {
        ...
        .then((res) => resolveResponse(
          removeActivityPermission(this.activityTemplateId, res.key.user_string_id),
        ))
        .then(() => {
          this.$message({
            showClose: true,
...
<style scoped>
.activity-template-poatac-panel-container {
  height: 100%;
  width: 100%;
  background: #FFFFFF;
}
