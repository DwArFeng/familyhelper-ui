<template>
  <div class="activity-template-file-panel-container">
      ...
      <header-layout-panel>
        <template v-slot:header>
          <div class="header-container">
            ...
            >
              <el-option
                v-for="activityTemplate in orderSelector.options"
                :key="activityTemplate.value"
                :label="activityTemplate.label"
                :value="activityTemplate.value"
              />
            </el-select>
            <div v-if="mode==='DEFAULT'" style="flex-grow: 1"/>
            <el-button
              class="activityTemplate icon-button"
              v-if="mode==='DEFAULT'"
              type="info"
...
export default {
  ...
  methods: {
    ...
    lookupDefault() {
      resolveResponse(childForActivityTemplate(
    ...
    lookupInspectedDateDesc() {
      resolveResponse(childForActivityTemplateInspectedDateDesc(
        this.activityTemplateId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
    ...
    lookupModifiedDateDesc() {
      resolveResponse(childForActivityTemplateModifiedDateDesc(
        this.activityTemplateId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
    ...
    lookupOriginNameAsc() {
      resolveResponse(childForActivityTemplateOriginNameAsc(
        this.activityTemplateId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
    ...
    lookupCreatedDateAsc() {
      resolveResponse(childForActivityTemplateCreatedDateAsc(
        this.activityTemplateId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
    ...
    handleFileInspect(row) {
      this.$router.push({
        ...
        query: { type: LIFE_ACTIVITY_TEMPLATE_FILE, id: row.key.long_id, action: 'inspect' },
      });
    },
    ...
    handleFileEdit(row) {
      this.$router.push({
        name: 'miscellaneous.fileEditor',
        query: { type: LIFE_ACTIVITY_TEMPLATE_FILE, id: row.key.long_id, action: 'edit' },
      });
    },
    ...
    handleUploadConfirmed(files, callback) {
      ...
      files.forEach((file) => {
        ...
        promises.push(resolveResponse(upload(this.activityTemplateId, formData)));
      });
      Promise.all(promises)
        ...
        .then(() => {
          this.handleSearch();
          this.$emit('onActivityTemplateFileUpdated');
        })
        .then(() => {
          callback(true);
        })
    ...
    handleCreateConfirmed(file, callback) {
      const formData = new FormData();
      formData.append('file', file.blob, file.name);
      resolveResponse(upload(this.activityTemplateId, formData))
        .then(() => {
          this.$message({
            showClose: true,
            message: '文件新建成功',
...
<style scoped>
.activity-template-file-panel-container {
  height: 100%;
  width: 100%;
  background: #FFFFFF;
}
