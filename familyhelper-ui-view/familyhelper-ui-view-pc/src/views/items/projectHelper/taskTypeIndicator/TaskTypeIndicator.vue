<template>
  <div class="task-type-indicator-container">
    <border-layout-panel
    ...
    >
      <el-form-item label="任务类型" prop="string_id">
        <el-input
...
} from '@/api/project/taskTypeIndicator';
  ...
  methods: {
    ...
    handleEntityCreate() {
      resolveResponse(insert(
        ...
        .then(() => {
          this.$message({
            showClose: true,
            message: `任务类型 ${this.anchorEntity.string_id} 创建成功`,
            type: 'success',
            center: true,
          });
        })
    ...
    handleEntityEdit() {
      resolveResponse(update(
        ...
        .then(() => {
          this.$message({
            showClose: true,
            message: `任务类型 ${this.anchorEntity.string_id} 更新成功`,
            type: 'success',
            center: true,
          });
        })
    ...
    handleEntityDelete(node, entity) {
      Promise.resolve(entity.key.string_id)
        .then((res) => this.$confirm('此操作将永久删除此任务类型。<br>'
          + '如果已经有任务属于这个类型，那么该银行的类型将会变为"（未知）"！<br>'
          + '是否继续?',
        '提示', {
