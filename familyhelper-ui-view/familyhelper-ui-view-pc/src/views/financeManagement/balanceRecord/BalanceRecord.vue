...
export default {
  ...
  methods: {
    ...
    lookupAllBankCardTypeIndicator() {
      resolveResponse(this, allBankCardTypeIndicator(0, 1000))
    ...
    lookupChildForAccountBook() {
      resolveResponse(this, childForAccountBookDisp(this.parentSelection.accountBookId, 0, 1000))
    ...
    handleBankCardToRecord() {
      this.$refs.recordForm.validate((accept) => {
        if (accept) {
          resolveResponse(this, recordBalance(
    ...
    handleBankCardRollback(index, item) {
      this.$confirm(`此操作撤销银行卡 ${item.name} 所记录的余额。<br>是否继续?`,
        ...
        .then(() => resolveResponse(this, rollbackBalance(item.key.long_id)))
        .then(() => {
          this.$message({
            showClose: true,
    ...
    handleRecordCommit() {
      Promise.resolve(this.parentSelection.accountBook)
        ...
        .then((res) => resolveResponse(this, recordCommit(res.key.long_id))
    ...
    handleRollbackAll() {
      Promise.resolve(this.parentSelection.accountBook)
        ...
        .then((res) => resolveResponse(this, rollbackAll(res.key.long_id))
          .then(() => Promise.resolve(res)))
        .then((res) => {
          this.$message({
            showClose: true,
