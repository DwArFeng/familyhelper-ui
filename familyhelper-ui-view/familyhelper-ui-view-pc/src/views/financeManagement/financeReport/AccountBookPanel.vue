...
export default {
  ...
  methods: {
    ...
    updateChart(accountBookKey) {
      this.chart.category = [];
      this.chart.data = [];
      resolveResponse(this, childForAccountBookDesc(accountBookKey, 0, 20))
        .then((res) => {
    ...
    handleBankCardSearch(accountBookId) {
      resolveResponse(this, bankCardChildForAccountBookDisp(
        accountBookId, this.bankCardTable.currentPage, this.bankCardTable.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.current_page > res.total_pages && res.total_pages > 0) {
            return resolveResponse(this, bankCardChildForAccountBookDisp(
              accountBookId, this.bankCardTable.currentPage, this.bankCardTable.pageSize,
            ));
          }
    ...
    handleFundChangeSearch(accountBookId) {
      ...
      resolveResponse(this, fundChangeChildForAccountBookDescDisp(
        ...
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.current_page > res.total_pages && res.total_pages > 0) {
            return resolveResponse(this, fundChangeChildForAccountBookDescDisp(
              accountBookId, this.fundChangeTable.currentPage, this.fundChangeTable.pageSize,
            ));
          }
          return Promise.resolve(res);
