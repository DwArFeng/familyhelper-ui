...
export default {
    ...
    const objectUserIdValidator = (rule, value, callback) => {
      Promise.resolve(value)
        .then((res) => {
          ...
          return resolveResponse(this, existsAccount(value));
        })
        .then((res) => {
          if (!res) {
          ...
          return resolveResponse(this, exists(this.me, value));
        })
        .then((res) => {
          if (res) {
  ...
  methods: {
    ...
    lookupAllAccount() {
      resolveResponse(this, allAccount(0, 1000))
    ...
    lookupChildForSubjectUser() {
      resolveResponse(this, childForSubjectUser(
        this.me, this.tablePanel.currentPage, this.tablePanel.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.current_page > res.total_pages && res.total_pages > 0) {
            return resolveResponse(this, childForSubjectUser(
    ...
    handleEntityDelete(node, entity) {
      this.$confirm('是否继续?', '提示', {
        ...
        .then(() => resolveResponse(this, remove(this.me, entity.key.object_user_id)))
        .then(() => {
          this.$message({
            showClose: true,
            message: '昵称删除成功',
