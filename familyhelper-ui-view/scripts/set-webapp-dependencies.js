...
run()
    ...
    .catch(e => {
        LogUtil.error(`设置 webapp 依赖失败, 将异常退出, 异常信息如下: ${e.message}`);
        LogUtil.error(`\n${e.stack}`);
        process.exit(1);
    })
...
async function executeSinglePackage(packageInfo) {
    ...
    if (!pkg.scripts || !pkg.scripts['set-webapp-dependencies']) {
        return
    }
    ...
    try {
        await ExecUtil.spawn(
            'pnpm',
            ['set-webapp-dependencies'],
            PathUtil.parsePath(`packages/${packageName}`),
        )
    } catch (e) {
        LogUtil.error(`包 ${packageName} 执行设置 webapp 依赖脚本失败, 将抛出异常, 异常信息如下: ${e.message}`);
        LogUtil.error(`\n${e.stack}`);
        throw e;
    }
}
