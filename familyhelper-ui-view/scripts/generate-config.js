...
async function executeSinglePackage(packageInfo) {
  ...
  const pkg = JSON.parse(
    readFileSync(PathUtil.parsePath(`packages/${packageName}/package.json`), 'utf-8'),
  )
  ...
  // 执行生成配置脚本。
  try {
    await ExecUtil.spawn('pnpm', ['generate-config'], PathUtil.parsePath(`packages/${packageName}`))
  } catch (e) {
