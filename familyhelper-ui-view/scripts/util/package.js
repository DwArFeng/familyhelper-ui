...
function parsePackageInfos() {
     ...
     * @type {string[]}
     */
    const pkgDirNames = fs
        .readdirSync(PathUtil.parsePath('packages'))
        .filter(f => {
            return fs.statSync(PathUtil.parsePath(`packages/${f}`)).isDirectory() &&
    ...
    for (const pkgDirName of pkgDirNames) {
         ...
         */
        const pkg = JSON.parse(readFileSync(PathUtil.parsePath(`packages/${pkgDirName}/package.json`), 'utf-8'));
