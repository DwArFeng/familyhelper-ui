...
本节集中维护 store 模块中的特殊字段约定；若模块在 Store 上暴露这些字段，框架或工具会按约定使用，便于查阅与扩展。
...
取值时， 若 store 的 `ready` 为 ref 形对象（即 `typeof value === 'object' && value !== null && 'value' in value`），
...
可参考 `./modules/navigation.ts`、`./modules/visualizer.ts` 的实现。

以下示例仅展示 `ready` 字段在模块中的定义与设置方式。

`./modules/exampleReady.ts`（片段）：

```ts
import { type StoreSetup } from '@/store/types.ts'

import { computed, type ComputedRef, ref } from 'vue'

// Store 类型中声明 ready。
export type ExampleReadyStore = {
...
function provideStoreSetup(): StoreSetup {
  ...
  return (): ExampleReadyStore => ({
    ready
  })
}

// 在初始化钩子中，使用延迟模拟加载行为，并在完成后设置 ready。
...
模块需要默认导出一个 `VimStoreModule` 类型的对象，VIM 会自动加载该对象，
开发人员可以参考 `./types.ts` 文件中的 `VimStoreModule` 类型定义，了解该对象的具体结构，并提供对应的方法实现。
开发人员可为添加的模块定义特殊字段（如 `ready`），约定与用法见上方「特殊字段」一节。
