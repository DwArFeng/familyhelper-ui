<template>
  <div class="activity-participant-panel-container">
...
import {
  ...
  update,
  remove,
} from '@dwarfeng/familyhelper-ui-component-api/src/api/life/activityParticipant';
import resolveResponse from '@/util/response';

// noinspection JSAnnotator
export default {
  name: 'ActivityParticipantPanel',
  components: {
    AccountSelector,
  ...
  data() {
    const keyValidator = (rule, value, callback) => {
      Promise.resolve(value)
        ...
        .then((res) => {
          if (res) {
            callback(new Error('该用户已经是参与者了哦'));
            return Promise.reject();
          }
...
<style scoped>
.activity-participant-panel-container {
  height: 100%;
  width: 100%;
  background: #FFFFFF;
}
