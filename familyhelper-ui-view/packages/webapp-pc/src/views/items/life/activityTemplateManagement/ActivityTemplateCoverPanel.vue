<template>
  <div class="activity-template-cover-panel-container">
...
import {
  ...
  upload,
  updateOrder,
} from '@dwarfeng/familyhelper-ui-component-api/src/api/life/activityTemplateCover';
...
export default {
  name: 'ActivityTemplateCoverPanel',
  ...
  methods: {
    handleCarouselSearch() {
      ...
      resolveResponse(childForActivityTemplate(this.activityTemplateId, 0, 100))
        .then((res) => {
          const promises = [];
          res.data.forEach((activityTemplateCoverInfo) => {
            promises.push(download(activityTemplateCoverInfo.key.long_id));
          });
    ...
    handleUpdateOrderDialogTableSearch() {
      ...
      resolveResponse(childForActivityTemplate(this.activityTemplateId, 0, 100))
        .then((res) => {
          const promises = [];
          res.data.forEach((activityTemplateCoverInfo) => {
            promises.push(download(activityTemplateCoverInfo.key.long_id).then(((blob) => {
              const neoActivityTemplateCoverInfo = activityTemplateCoverInfo;
              // noinspection JSCheckFunctionSignatures
              neoActivityTemplateCoverInfo.url = window.URL.createObjectURL(blob);
              return Promise.resolve(neoActivityTemplateCoverInfo);
            })));
          });
          return Promise.all(promises);
        })
    ...
    handleCoverDelete(index, entity) {
      this.$confirm('此操作将永久删除此活动模板封面。<br>是否继续?',
        '提示', {
...
<style scoped>
.activity-template-cover-panel-container {
  width: 100%;
  height: 100%;
