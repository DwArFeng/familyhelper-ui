...
} from '@dwarfeng/familyhelper-ui-component-api/src/api/life/poad';
import {
  removePermission,
  upsertPermission,
} from '@dwarfeng/familyhelper-ui-component-api/src/api/life/activityDataSet';
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      this.lookupChildForActivityDataSet();
    },
    lookupChildForActivityDataSet() {
      this.loading = true;
      resolveResponse(childForActivityDataSetDisp(
        this.activityDataSetId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
    ...
    handleDeleteButtonClicked(index, row) {
      ...
      Promise.resolve(row)
        .then((res) => this.$confirm(
          `此操作将删除 ${row.account.display_name}(${row.account.key.string_id})
          对该活动数据集合的权限。<br>是否继续?`,
          '提示', {
        ...
        .then((res) => resolveResponse(
          removePermission(this.activityDataSetId, res.key.user_string_id),
        ))
        .then(() => {
          this.$message({
            showClose: true,
    ...
    handleUpsertButtonClicked() {
      resolveResponse(upsertPermission(
        this.activityDataSetId, this.form.userId, this.form.permissionLevel,
      ))
        .then(() => {
          this.$message({
            showClose: true,
