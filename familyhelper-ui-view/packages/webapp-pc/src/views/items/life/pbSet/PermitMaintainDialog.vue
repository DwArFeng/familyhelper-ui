...
import { childForPbSetDisp } from '@dwarfeng/familyhelper-ui-component-api/src/api/life/popb';
import {
  removePermission,
  upsertPermission,
} from '@dwarfeng/familyhelper-ui-component-api/src/api/life/pbSet';
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      this.lookupChildForPbSet();
    },
    lookupChildForPbSet() {
      resolveResponse(childForPbSetDisp(
        this.pbSetId, this.table.currentPage, this.table.pageSize,
      ))
        .then((res) => {
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.current_page > res.total_pages && res.total_pages > 0) {
            return resolveResponse(childForPbSetDisp(
              this.pbSetId, res.total_pages - 1, this.table.pageSize,
            ));
          }
    ...
    handleDeleteButtonClicked(index, row) {
      ...
      Promise.resolve(row)
        ...
        .then((res) => resolveResponse(removePermission(this.pbSetId, res.key.user_string_id)))
        .then(() => {
          this.$message({
            showClose: true,
            message: '权限删除成功',
    ...
    handleUpsertButtonClicked() {
      resolveResponse(upsertPermission(
        this.pbSetId, this.form.userId, this.form.permissionLevel,
      ))
        .then(() => {
          this.$message({
            showClose: true,
