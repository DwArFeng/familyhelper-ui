<template>
  <div class="bank-card-type-indicator-container">
    ...
    >
      <el-form-item label="银行卡类型" prop="string_id">
        <el-input
...
} from '@dwarfeng/familyhelper-ui-component-api/src/api/finance/bankCardTypeIndicator';
  ...
  methods: {
    ...
    handleEntityCreate() {
      resolveResponse(insert(
        ...
        .then(() => {
          this.$message({
            showClose: true,
            message: `银行卡类型 ${this.anchorEntity.string_id} 创建成功`,
            type: 'success',
            center: true,
          });
        })
    ...
    handleEntityEdit() {
      resolveResponse(update(
        ...
        .then(() => {
          this.$message({
            showClose: true,
            message: `银行卡类型 ${this.anchorEntity.string_id} 更新成功`,
            type: 'success',
            center: true,
          });
        })
    ...
    handleEntityDelete(node, entity) {
      Promise.resolve(entity.key.string_id)
        .then((res) => this.$confirm('此操作将永久删除此银行卡类型。<br>'
