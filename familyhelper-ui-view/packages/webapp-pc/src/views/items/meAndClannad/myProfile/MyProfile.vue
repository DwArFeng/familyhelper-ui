...
import {
  childForCategory,
} from '@dwarfeng/familyhelper-ui-component-api/src/api/clannad/profileTypeIndicator';
import {
  all as allAccount,
  childForProfileGuest,
...
export default {
  ...
  methods: {
    ...
    lookupSubCategoryIndicator(category, indicatorObject) {
      resolveResponse(childForCategory(category, 0, 100))
        .then((res) => {
    ...
    handleProfileReset() {
      resolveResponse(inspect(this.me))
    ...
    handleResume() {
      this.$confirm('此操作将丢弃未保存的更改。<br>是否继续?',
        ...
        .then(() => resolveResponse(inspect(this.me)))
        .then(this.updateFormEntity)
        .then(() => {
          this.$message({
            showClose: true,
    ...
    handleResetGuestRelation() {
      ...
      resolveResponse(resetGuestPermission(guestKeys))
        .then(() => {
          this.$message({
            showClose: true,
            message: '访客权限更新成功',
