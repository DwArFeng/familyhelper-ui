...
} from '@dwarfeng/familyhelper-ui-component-api/src/api/system/role';
...
export default {
  ...
  methods: {
    ...
    lookupAllAccount() {
      resolveResponse(
        allAccount(this.accountTablePanel.currentPage, this.accountTablePanel.pageSize),
      )
        .then((res) => {
    ...
    handleAccountDelete(node, entity) {
      Promise.resolve()
        ...
        .then(() => {
          const accountIdToDelete = entity.key.string_id;
          return resolveResponse(remove(accountIdToDelete));
        })
        .then(() => {
    ...
    handleResetRoleRelation() {
      Promise.resolve()
        ...
        .then(() => {
          ...
          return resolveResponse(resetRoleRelation(accountKey, roleKeys));
        })
        .then(() => {
      ...
      this.$refs.passwordResetForm.validate((accept) => {
        ...
        Promise.resolve()
          ...
          .then(() => {
            ...
            return resolveResponse(resetPassword(accountIdToEdit, newPassword));
          })
          .then(() => {
            const accountId = this.accountTablePanel.selection.key.string_id;
            const { name } = this.accountTablePanel.selection;
