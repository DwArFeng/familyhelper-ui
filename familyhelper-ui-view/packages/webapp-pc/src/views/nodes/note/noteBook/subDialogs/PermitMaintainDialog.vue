      ...
      <header-layout-panel class="dialog-container" v-loading="loading">
          ...
          <div class="header-container">
            <el-form class="header-form" :inline="true" :model="form">
              ...
              <el-form-item class="header-form-item">
                <el-button type="primary" :disabled="form.userId === ''" @click="handlePonbUpsert">
                  添加/更改
                </el-button>
              </el-form-item>
            </el-form>
        ...
        <template v-slot:default>
          <table-panel
            ...
            v-model:current-page="ponbTableCurrentPage"
            v-model:page-size="ponbTablePageSize"
            :item-count="ponbTableItemCount"
            :page-sizes="[15, 20, 30, 50]"
            :items="ponbTableItems"
            :inspect-button-visible="false"
            :edit-button-visible="false"
            :delete-button-visible="false"
            :operate-column-width="53"
            @onPagingAttributeChanged="handlePonbTablePagingAttributeChanged"
          >
            <template v-slot:default>
              <el-table-column
                prop="account"
            ...
            <template v-slot:operateColumn="{ row }">
              <el-button
                ...
                @click="handlePonbDelete(row as DispPonb)"
              />
            </template>
          </table-panel>
        </template>
...
const form = ref<{
  userId: string
  permissionLevel: PonbPermissionLevel
}>({
  userId: '',
  permissionLevel: 1,
})
const formPermissionLevelIndicator = ref<
  { key: PonbPermissionLevel; label: string; selectable: boolean }[]
>([
  { key: 0, label: '所有者', selectable: false },
  { key: 1, label: '访客', selectable: true },
])

async function handlePonbUpsert(): Promise<void> {
  loading.value += 1
  try {
    if (!props.noteBookId) {
    ...
    await resolveResponse(
      upsertPermission({
        note_book_key: { long_id: props.noteBookId },
...
function handlePonbSearch(): void {
  if (!props.noteBookId) {
    return
  }
  handlePonbChildForNoteBookSearch()
}

async function handlePonbChildForNoteBookSearch(): Promise<void> {
  loading.value += 1
  try {
    if (!props.noteBookId) {
      throw new Error('不应该执行到此处，请联系开发人员')
    ...
    const res = await lookupWithAdjustPage(
      (pagingInfo) => childForNoteBookDisp({ long_id: props.noteBookId }, pagingInfo),
      ponbTablePagingInfo.value,
    )
    updatePonbTableByLookup(res)
  } finally {
    loading.value -= 1
  }
}
...
// -----------------------------------------------------------笔记本权限表格处理-----------------------------------------------------------
const {
  currentPage: ponbTableCurrentPage,
  pageSize: ponbTablePageSize,
  itemCount: ponbTableItemCount,
  items: ponbTableItems,
  pagingInfo: ponbTablePagingInfo,
  updateByLookup: updatePonbTableByLookup,
} = useIdentityBackendPagingTablePanel<DispPonb>(15)

// 此处 any 是 element-plus API 规定的，故忽略警告。
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function accountFormatter(row: DispPonb, column: any): string {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// 此处 any 是 element-plus API 规定的，故忽略警告。
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function permissionLevelFormatter(row: DispPonb, column: any): string {
...
function handlePonbTablePagingAttributeChanged(): void {
  handlePonbSearch()
}

async function handlePonbDelete(row: DispPonb): Promise<void> {
  try {
