<template>
  <div class="navigation-settings-remote-panel">
    <loading-overlay :loading="loading > 0 && !remoteKeyEditDialogVisible" />
    ...
    <maintain-dialog
      v-model:visible="remoteKeyEditDialogVisible"
      label-width="100px"
      edit-title="设置远端 Navigation Key"
      edit-confirm-button-label="确认"
      :loading="remoteKeyEditDialogLoading"
      :mode="remoteKeyEditDialogMode"
      :item="remoteKeyEditDialogItem"
      :close-on-click-modal="false"
      :width="480"
      @on-item-edit="handleRemoteKeyEdit"
    >
      <native-form-item label="类型">
        <native-select
          v-model="remoteKeyEditDialogItem.type"
          class="dialog-select"
          :disabled="remoteKeyEditDialogLoading > 0"
          :options="remoteKeyTypeSelectOptions"
        />
      </native-form-item>
      <native-form-item v-if="remoteKeyEditDialogItem.type === 'custom'" label="自定义 ID">
        <div class="custom-id-row">
          <input
            v-model="remoteKeyEditDialogItem.customId"
          ...
          <native-button
            kind="primary"
            :disabled="remoteKeyEditDialogLoading > 0"
            @click="openRemoteKeyCustomNavDialog"
          >
            选择
          </native-button>
        </div>
    ...
    <custom-navigation-select-dialog
      v-model:visible="remoteKeyCustomNavDialogVisible"
      @on-confirmed="handleRemoteKeyCustomNavConfirmed"
    />
  </div>
</template>
...
const SETTINGREPO_CATEGORY = 'public$framework.pc.navigation'
...
// region 远端 Navigation Key 编辑对话框

type RemoteKeyEditDialogItem = {
  type: NavigationKeyType
  customId: string
}

const INITIAL_REMOTE_KEY_EDIT_ITEM: RemoteKeyEditDialogItem = {
  type: 'default',
  customId: '',
}

function navigationKeyToRemoteKeyEditItem(key: NavigationKey): RemoteKeyEditDialogItem {
  return {
    type: key.type,
...
const { item: remoteKeyEditDialogItem, mode: remoteKeyEditDialogMode } = useEditOnlyMaintainDialog<
  NavigationKey,
  RemoteKeyEditDialogItem
>(navigationKeyToRemoteKeyEditItem, INITIAL_REMOTE_KEY_EDIT_ITEM)

const remoteKeyEditDialogLoading = ref<number>(0)

const remoteKeyTypeSelectOptions: NativeSelectOption[] = [
  { label: '内置默认导航', value: 'default' },
  { label: '自定义导航', value: 'custom' },
]

const remoteKeyEditDialogVisible = ref(false)

watch(
  () => [remoteKeyEditDialogVisible.value, displayedKey.value] as const,
  ([visible, key]) => {
    if (visible) {
      remoteKeyEditDialogItem.value = navigationKeyToRemoteKeyEditItem(key)
    }
  },
)

function buildKeyFromRemoteKeyEditItem(item: RemoteKeyEditDialogItem): NavigationKey | null {
...
async function handleRemoteKeyEdit(item: RemoteKeyEditDialogItem): Promise<void> {
  const key = buildKeyFromRemoteKeyEditItem(item)
  if (!key) {
    return
  }
  let payload: string
  ...
  remoteKeyEditDialogLoading.value += 1
  try {
    ...
    remoteKeyEditDialogVisible.value = false
    await handleRefresh()
  } catch (e: unknown) {
    let message: string
    if (e instanceof Error) {
...
function openEditDialog(): void {
  remoteKeyEditDialogVisible.value = true
}
...
const remoteKeyCustomNavDialogVisible = ref(false)

function openRemoteKeyCustomNavDialog(): void {
  remoteKeyCustomNavDialogVisible.value = true
}

function handleRemoteKeyCustomNavConfirmed(row: CustomNavigation): void {
  remoteKeyEditDialogItem.value.customId = row.key
}
