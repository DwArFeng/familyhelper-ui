...
在某些场景，尤其是需要根据不同的 component key 动态加载组件时，开发人员可能需要使用 compreg 模块提供的 API 来获取组件。
...
function getDefaultComponent(): void {
  const compreg: Omit<VimCompreg, 'init'> = vim.ctx().compreg()
  const component = compreg.defaultComponent()
  console.log(component)
}
```

以根据 key 获取组件为例，代码如下所示：
...
function getComponentByKey(): void {
  const compreg: Omit<VimCompreg, 'init'> = vim.ctx().compreg()
  const component = compreg.component('welcome')
  if (component) {
    console.log(component)
  }
}
...
- `key`：组件的唯一标识符，用于通过 `compreg.component(key)` 方法获取组件。
